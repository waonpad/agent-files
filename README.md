...
| `.claude/hooks/`              | フックのスクリプトやアセット。`.github/hooks/` からシンボリックリンクで参照される    |
...
| `.github/hooks/`                      | `.claude/hooks/` へのシンボリックリンク                      |
...
#### .claude/hooks/ と .github/hooks/

フックが参照するスクリプトやアセット（lint.sh など）を共有するため。  
...
`.claude/skills/sync-agents/` のスキルにより、エージェント定義を変換しながら相互同期する。

#### フック定義

フックのイベント名・フィールド名がツール固有のため個別管理する。  
ただしアセットはツール共通のため `.claude/hooks/` で一元管理する。

##### 同期のための仕組み

`.claude/skills/sync-hooks/` のスキルにより、イベント名・フィールド名を変換しながら相互同期する。
